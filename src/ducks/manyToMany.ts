...
export type Action = { type: ActionType; start: string; end: string; name: string; params: bus.ManyToManyEdgeInput };
...
export const reducer = (schema: bus.Schema, action: Action): bus.Schema => {
  ...
  return bus.utils.addEdge(schema, { ...params, name, start, end, kind: bus.EDGE.manyToMany });
};
